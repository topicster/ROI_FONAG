...
class modelinit():
    ...
    def contaminantes(self, path):
        ...
        for i in lines_cnt[1:]:
            ...
            diccnt['cnt%s' % i[0]] = {'r_as': r_as, 'r_at': r_at, 'r_amn': r_amn, 'decay': i[1],
                                      'out_data_cnt': out_data_cnt}  # , 'cntset': cntset}
        print('Fin bucle')
    ...
    def balance(self, path):
        ...
        # Agregar areas de aporte
        ExtractMultiValuesToPoints(out_data_qnt, [[r_area, 'AREA']])
        ExtractMultiValuesToPoints(out_data_qds, [[r_area, 'AREA']])
        ...
        for i in range(iter):
            ...
            Qd = Con(Qn + (r_demand_mmdia * float(ndias[mes])) <= 0, 0, Qn + (r_demand_mmdia * float(ndias[mes])))
            ...
            r_qds_m3s = FlowAccumulation(r_FlowDirection, Qd_m3s)
            ...
            if cont:
                diccnt = self.transporte(q, n, ndias, mes, y, m, diccnt, factorh, r_qds_m3s, r_mes,
        ...
        if cont:
            ...
            for j in diccnt:
                arcpy.TableToTable_conversion(diccnt[j]['out_data_cnt'],
                                              '%s/modeloFONAG/%s/tables/' % (path, t_inputs[16]),
                                              't_out_data_%s.csv' % j)
    ...
    def transporte(self, q, n, ndias, mes, y, m, diccnt, factorh, r_qds_m3s, r_mes, inMaskData, cellArea,
                   r_FlowDirection, path):
